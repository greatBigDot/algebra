...
||| \mathbb{Z}^2 \to \mathbb{Z}\\), and (c) two unique elements of it, \\(0\\)
||| and \\(1\\). There are various obvious properties that universally hold;
||| e.g.:
|||
||| * \\(x+(y+z) = (x+y)+z\\), and \\(x\cdot (y \cdot z) = (x \cdot y) \cdot
||| z\\)
...
||| asymmetry in the fourth law, an asymmetry not present in the others. This
||| is necessary, as the following property (referred to as **multiplicative
...
||| set, to calculate the \"sum\" of two elements, we calculate their usual
||| sum, then take that modulo 5 (that is, the remainder when dividing by 5;
||| the modulo operation is essentially a generalization of the concept of even
||| and odd numbers to divisibilities other than by 2); similarly with the
||| product. Thus, for example, \\(\overline{4} + \overline{3} =
||| \overline{2}\\). A similar line of reasoning indicates that additive
||| inverses exist\-\--in fact, so do multiplicative inverses, at least for
...
||| should be enough to give a taste of what abstract algebra is all about. You
||| start with some set with defined operations (a pairing known as a
||| \"structure\") that you are familiar with, you distill that structure down
||| to its most basic and fundamental properties, and then you look at what
||| sorts of things satisfy those properties and what those properties alone
||| entail. This allows you to generalize familiar structures to more abstract
||| ideas\-\--the original motivating structure becomes just a special case of
||| a more general property, and seemingly disparate things suddenly become two
||| special cases of a more general idea.
|||
...
||| are entailed by them, and which properties will differ between different
...
||| simplest way to divide those that do always admit a unique factorization
||| and those that don't? These are the kinds of questions that the framework
||| of algebraic structures allows you to ask.
|||
...
||| they relate to each other (namely, via __ring homomorphisms__), and how
||| they relate to other mathematical objects. Rings are not the only algebraic
||| structure; there are many, many more, and this package defines a few of the
||| more important ones as Idris interfaces, as well as some instances of that
||| interface. Each module contains in its documentation an overview of that
||| algebraic structure and some interesting properties of them that make them
||| worth studying.
|||
